...
public class SequenceSensitivityEvaluator {
    ...
    public SequenceSensitivityEvaluator() {
...
//            if (solution.equals("flute")) {
            ...
            for (int i = 0; i < 10; ++i) {
                RR += 1.0 * (supportedTestCorrect[fold][i] + nonSupportedTestCorrect[fold][i]) / (i+1);
            }
            for (int i = 1; i < 10; ++i) {
    ...
    public void writeResult(String solution, int fold)  {
        try {
            ...
            String[] MRR = {"MRR", String.format("%.2f", RR / (supportedTest[fold] + nonSupportedTest[fold]))};
            result.add(MRR);
    ...
    public static void main(String[] args) {
        ...
        ModelConfig.tokenizedType = ModelConfig.TokenizedType.FULL_TOKEN;
        TestConfig.predictType = TestConfig.PredictType.SEQUENCE;
        TestConfig.fold = "9";
        ...
        for(int i = 0; i < 10; ++i) {
            if (i % 2 == 0) folds.add(i);
        }
        for (String proj: projects) {
            for(int fold: folds) {
                TestConfig.lastParcFold = fold;
                ProjectConfig.project = proj;
                Config.init();
                SequenceEvaluator sequenceEvaluator = new SequenceEvaluator();
                sequenceEvaluator.evaluate(proj, "flute", 9);
            }
        }
    }
}
