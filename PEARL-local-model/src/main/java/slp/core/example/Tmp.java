...
import flute.communicating.MultipleParamRequest;
 ...
 */
public class Tmp {
    public static String response(String request, Vocabulary vocabulary, ModelRunner modelRunner) {
//        modelRunner.getModel().notify();
        long start_time_test = System.nanoTime();
        JavaLexer lexer = (JavaLexer) modelRunner.getLexerRunner().getLexer();
        Gson gson = new Gson();
        MultipleParamRequest jsonRequest = gson.fromJson(request, MultipleParamRequest.class);

        // read context
        ArrayList<String> context = (ArrayList<String>) jsonRequest.lex_context;
        ArrayList<String> contextParseAgain = new ArrayList<>();
        for (String s : context) {
            contextParseAgain.addAll(lexer.tokenizeLines(s).get(0));
        }
//        System.out.println(contextParseAgain);

        List<Integer> contextInt = vocabulary.toIndices(contextParseAgain);

        // read candidates
        ArrayList<String> recs = new ArrayList<>();
//        System.out.println(jsonRequest.next_lex.size());
        for (List<List<String>> paramPosition : jsonRequest.next_lex) {
            for (List<String> excodeParam : paramPosition) {
                recs.addAll(excodeParam);
            }
        }
        recs = new ArrayList<>(new HashSet<>(recs));
        // score candidates
        int contextLen = contextInt.size();
        ArrayList<Pair<String, Double>> scores = new ArrayList<>();
        for (String rec : recs) {
            List<String> recTokens = lexer.tokenizeLines(rec).get(0);
//            System.out.println(recTokens);
            List<Integer> recTokensInt = vocabulary.toIndices(recTokens);
            contextInt.addAll(recTokensInt);
            List<Map<Integer, Pair<Double, Double>>> predictions = modelRunner.getModel().predict(contextInt);

            Double score = (double) -1;
            for (int i = contextLen; i < contextInt.size(); ++i) {
                Map<Integer, Pair<Double, Double>> mp = predictions.get(i);
                Pair<Double, Double> prob = mp.get(contextInt.get(i));
                if (prob != null) {
                    if (score < 0) {
                        score = prob.left;
                    }
                    else score = score * prob.left;
                }
            }
//            System.out.println(score);
            scores.add(new Pair<>(rec, score));
            for (int i = 0; i < recTokensInt.size(); ++i) {
                contextInt.remove(contextInt.size() - 1);
            }
        }
        scores.sort(Comparator.comparing(p -> -p.right));

        int top_k = 10;
        String response = "{\"type\":\"predict\", \"data\":{\"ngram\":{\"result\":";
        ArrayList<String> topCands = new ArrayList<>();
        for (int i = 0; i < Math.min(top_k, scores.size()); ++i) {
            topCands.add(scores.get(i).left);
        }
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        ...
        FileOutputStream f = new FileOutputStream("myObjects.model");
        ...
        FileInputStream fi = new FileInputStream("myObjects.model");
...
//		Stream<Pair<File, List<List<Double>>>> modeledFiles = modelRunner.predictDirectory(test);
        ...
        try {
            ...
            while (true) {
                ...
                String response = Tmp.response(jsonRequestString, vocabulary, modelRunner);
                os.write(response);
                os.newLine();
                os.flush();
            }
...
//        for (Map<Integer, Pair<Double, Double>> mp : stuff) {
//            System.out.println(mp.size());
//            for (Map.Entry<Integer, Pair<Double, Double>> entry : mp.entrySet()) {
//                System.out.print("(" + vocabulary.toWord(entry.getKey()) + " " + entry.getValue().left + " " + entry.getValue().right + ") ===");
//            }
//            System.out.println();
//        }


//		modelRunner.getModel().predict()
