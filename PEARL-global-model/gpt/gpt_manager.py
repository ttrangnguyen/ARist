...
from . import encoder, model, sample, preprocessor

TEST_MODE = False
TOP_K = 10
GPT_BATCH_SIZE = 2  # Set to 1 if PARAM_LEXICAL_ONLY is set to False
...
class GPTManager:
    ...
    def predict_param_using_lex(self, data):
        ...
        for i in range(n_param):
            ...
            for j in range(len(data['next_lex'][i])):
                for k in range(len(data['next_lex'][i][j])):
                    candidate_param = data['next_lex'][i][j][k]
                    candidate_param = preprocessor.empty_string_literal(candidate_param)
                    if "{" in candidate_param:
                        candidate_param = candidate_param[:candidate_param.index("{")].rstrip()
                    if "]" in candidate_param:
                        candidate_param = preprocessor.remove_array_access_index(candidate_param)
                    if "(" in candidate_param and candidate_param.index("(") > 0:
                        candidate_param = preprocessor.normalize_method_invocation(candidate_param)
                    candidates_param.append(candidate_param)
        ...
        for i in range(len(sorted_scores)):
            ...
            scores_gpt.append(sorted_scores[i][2])
                        ...
                        for t in range(len(out[i])):
                            ...
                            for c in range(len(token)):
                                ...
                                if token[c] == ")":
                                    ...
                                    break_i_flag = True
                                if token[c] == ">":
                                    prefix = self.encoder.decode(out[i][:t+1])
                                    if prefix.startswith("<COMPOUND>") or prefix.startswith("<LAMBDA>"):
                                        prob = prob[:t + 1]
                                        suggestion_java += token[:c + 1]
                                        suggestion_tokens.append(out[i][t])
                                        suggestion_tokens.append(comma_tokens[0])
